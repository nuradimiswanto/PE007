<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Layang-Layang</title>
</head>
<body>

    <a href="index.php">Home</a>
    <h1>Hitung Luas dan Keliling Layang-Layang</h1>

    <form action="layanglayangaction.php" method="post">
        <table>
            <tr>
                <td>Masukkan Diagonal 1:</td>
                <td><input type="number" name="d1" required step="any" min="0"></td>
            </tr>
            <tr>
                <td>Masukkan Diagonal 2:</td>
                <td><input type="number" name="d2" required step="any" min="0"></td>
            </tr>
            <tr>
                <td>Masukkan Sisi Pendek:</td>
                <td><input type="number" name="sisi1" required step="any" min="0"></td>
            </tr>
            <tr>
                <td>Masukkan Sisi Panjang:</td>
                <td><input type="number" name="sisi2" required step="any" min="0"></td>
            </tr>
            <tr>
                <td>Pilih Perhitungan:</td>
                <td>
                    <input type="radio" name="pilih" value="luas" required> <label>Luas</label><br>
                    <input type="radio" name="pilih" value="keliling"> <label>Keliling</label><br>
                    <input type="radio" name="pilih" value="semua"> <label>Semua</label>
                </td>
            </tr>
            <tr>
                <td></td>
                <td><input type="submit" value="Hitung"></td>
            </tr>
        </table>
    </form>

</body>
</html>